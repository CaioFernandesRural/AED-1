...
void porcentagemHash(Hash tab){
    ...
    // acho que é esse o cálculo
    printf("\n %.2f%% da lista cheia", (tam/100)*cont);
}

void indiceColisao(Hash tab){

    for (int i = 0; i < tam; i++){ //se tiver proximo
        if (tab[i]->prox != NULL){
            printf("\ncolisao na posicao %d", i);
        }
    }
}
