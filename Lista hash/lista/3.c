...
#define tam 11 //chaves base
#define n (2*tam)//tamanho máximo
...
typedef Dados* Hash[n]; //declaração de um array contendo nós, de tamanho n (2 vezes o tamanho)

int funcHash(int num){
    return (num % tam); //função que retorna o codigo hash
}
void inicializaHash(Dados* tab){
...
void insereHash(Dados *tab, int x){
    
    int chave = funcHash(x);//gera a chave hash a partir de x
    
    while (tab[chave].info != 0){
        chave = (chave + 1) % tam;
    }//função de hash linear
    tab[chave].info = x;
}
